...
if __name__ == "__main__":
    text = """
A palavra "bichinho" é um diminutivo carinhoso da palavra "bicho", usada no português, principalmente no Brasil. Pode ter vários significados, dependendo do contexto:

1- Animal pequeno ou fofinho:

Ex: "Olha aquele bichinho, que fofo!"
Pode se referir a um filhote ou animal de estimação.

2- Inseto ou criatura pequena:

Ex: "Tem um bichinho andando na parede."
às vezes usado para se referir a insetos, ácaros, ou outros pequenos seres.

3- Pessoa querida (modo afetuoso):

Ex: "Meu bichinho, você tá bem?"
Pode ser um apelido carinhoso entre casais, pais e filhos, ou amigos.

4- Ser imaginário ou simbólico:
Em algumas regiões, pode ser usado de forma simbólica para representar um sentimento:

"Tem um bichinho dentro de mim dizendo que algo vai dar errado."
    """

    pages = build_pages_colored(text)

    for idx, page_img in enumerate(pages):
        page_with_folds = simulate_paper_folds(page_img,
                                            warp_amplitude=5,
                                            warp_period=200,
                                            max_crease_intensity=0.2,
                                            crease_count=5,
                                            crease_shadow_width=30,
                                            global_noise_scale=0.05,
        ...
        cv2.imwrite(filename, page_with_folds)
