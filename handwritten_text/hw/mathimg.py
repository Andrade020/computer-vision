...
Primary backend: matplotlib mathtext (no external TeX needed, handles most
inline/display math). Optional backend: real MiKTeX via pdflatex if a PDF
rasterizer (PyMuPDF) is available -- fuller symbol coverage.

Returns an RGBA image with the given ink color, transparent background,
scaled so its visual height matches `height_px`.
"""
import io
import numpy as np
